...
```{r}
acedbase <- lm(acedminmod,ACEDextract,na.action=na.omit)
summary(acedbase)
```
```{r}
acedati <- lm(post_scaled ~ Cond_code * pre_scaled, ACEDextract,na.action=na.omit)
summary(acedati)
```
```{r}
...
```{r}
ACEDextract %>% mutate(control=Cond_code=="control") -> ACEDextract
acedcont <- lm(post_scaled ~ control + pre_scaled,ACEDextract, na.action=na.omit)
summary(acedcont)
```
