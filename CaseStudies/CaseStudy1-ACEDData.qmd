...
Other optional steps: \* A call to `stat_XXX()` to calculate statistics
\* A call to `scale_*_XXX()` to establish a scale, here `*` is an
aesthetic. \* A call to `facet_grid()` or `facet_wrap()` to put multiple
subplots \* Calls to `labs()`, `annotate()`, `guide()`, `theme()`.
