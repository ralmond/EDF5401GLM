...
### First Regression
```{r ACEDregression}
aced_lm <- lm(post_scaled ~ EAP.sgp, data=ACEDextract)
summary(aced_lm)
```
...
The accommodation given to English Language Learner (ELL) students was that their teachers
could translated any words they were having problems with.  However, the study team
noted that these translations often became instruction.
...
2. The students were randomly assigned to study conditions, and no Control student has
an `EAP.sgp` score (as they didn't use ACED).  Does this introduce a bias?

2.  Is the distribution the same for everybody in the group? Are there differences by
study condition (`Cond_code`)? class level (`Level_Code`)? class (`Session`)?

3.  What at the distributions of $X$ (`EAP.sgp`) and $Y$ (`post_scaled`) like? 
High skeweness? High kurtosis? Outliers?
...
7.  Is there enough evidence in the sample to conclude that $X$ and $Y$ are related? How strong is the relationship?

8.  What is the equation of the regression line?

9. The ELL students had slightly different administration conditions.  How sensitive are
the conclusions to those students?

10. What can we conclude about the relationship between $X$ and $Y$?  In particular, both
`EAP.sgp` and `post_scaled` are measures of the students ability to solve geometric 
sequence problems?  Are they measuring something similiar?
...
11. What can we conclude about the relationship between $X$ and $Y$?  In particular, does
increasing spending cause an increase in pay?  Increasing pay cause an increase in 
spending?  Or is there some common factor.
