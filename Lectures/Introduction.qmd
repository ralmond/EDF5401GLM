...
-   Coffee Hour:  TBD (Vote for time at:
https://doodle.com/poll/nc3qvaf2i2rs4qti?utm_source=poll&utm_medium=link)
...
Cohen, J., Cohen, P., West, S. & Aiken, L. S. (2003). _Applied multiple regression/correlation analysis for the behavioral sciences (3rd ed)_.  Lawrence Erlbaum Associates, Inc. ISBN 9780203774441
...
Lewis-Beck, C. and Lewis-Beck, M. S. (1980). _Applied regression: An introduction._ Sage Publications. (ISBN e-book: 9781483381480, Paperback: 9781483381473)
...
Gelman, A., Hill, J. & Vehtari, A. (2020).  _Regression and Other Stories._  Cambridge University Press.  URL: <https://avehtari.github.io/ROS-Examples/>  (This text is used for EDF 5484.)
...
* [Chapter 1](http://www.math.wm.edu/~leemis/otext1.pdf): Simple Linear Regression

* [Chapter 2](http://www.math.wm.edu/~leemis/otext2.pdf): Inference in Simple Linear Regression

* [Chapter 3](http://www.math.wm.edu/~leemis/otext2.pdf): Topics in Regression
...
[R and R Studio](https://pluto.coe.fsu.edu/rdemos/R_and_R_Studio_Presentation2.Rmd)
...
A _statistic_ is an operator which summarizes a data set.  This could be a numerical summary or a graphical summary.

_Statistics_ is the study of statiscs.

## What are Data?

Data (a plural noun) _are_ a collection of observations about a collection of subjects.
...
If the data set is $\textbf{X}$, then the value of Variable $j$ for Individual $i$ is $x_{i,j}$.  

* In R, use the expression `X[i,j]` to extract values from a matrix, data frame, or tibble.

[Matrixes are bold upper case, values in the matrix are lower case]

The vector $\boldsymbol{x}_i = (x_{i,1},\ldots,x_{i,J})$ is the set of all measurements on Individual $i$.  (Vectors are in bold face).
...
to both code and run.  Many statistic functions in R work the same way, e.g., `mean()`, `median()`, `sd()`.
...
R only provides built-in `rowXXX` function for mean and sums, but the other
two methods work with any function.
...
Often a distribution has parameters (usually written as Greek letters).  
  ...
  * $X \sim N(\mu,\sigma)$ -- $X$ is normally distributed with mean $\mu$
  and standard deviation $\sigma$.  (Check carefully, authors sometimes
  uses variance,  $\sigma^2$ or precision $\sigma^{-2}$ instead of s.d.)
  
A joint distribution is over two or more variables:

$F(x,y) = \Pr(X \leq x \wedge Y \leq y)$, where $\wedge$ means _and_.
...
So if $t(\textbf{X})$ is a statistic, write $\hat t$ or $\tilde t$ for estimate.
...
* Missing Data
  ...
  - _Missing Completely at Random_ -- complete cases are a SRS of all data.
  
    - Complete case analysis works.
  
  - _Missing at Random_ -- complete cases are a stratified sample of all data
    - Can use various techniques to adjust for missingness
    
  - _Non-ignorable Missingness_ --  need a more complex model for missingness.
  

## Properties of an estimate

A statistic $T$ from a sample is a random variable.  Let $T^{*}$ be
the value of the statistic in the population.

* bias -- a systematic error, $E[T-T^{*}]$

* standard error -- standard deviation of the statistic (if we could take
the sample over and over again), $s_{T} = \sqrt{\textrm{Var}(T)}$

* robustness/resistance -- how sensitive the statistic is to outliers or
...
A _representative sample_ is one which will produce an unbiased estimate.
...
$Y$ is the _dependent variable_, the target of prediction.
...
This model is a line;  $b_0$ is the intercept and $b_1$ is the slope.
In general, $f(\boldsymbol{x}) = b_0 + b_1 x_1 + b_2 x_2 + \cdots$ is a _linear predictor_.

The _fitted value_ or _predicted value_ for $Y_i$ is $f(\boldsymbol{x}_i)$, often written $\hat {Y_i}$ or $\tilde {Y_i}$.
...
Can define a _loss function_, $L(Y,f(\boldsymbol{X}|\boldsymbol{\beta}))$.
...
The values of $\boldsymbol{\beta}$ which minimize the square error,
are the least square estimate, $\hat{\boldsymbol{\beta}}$.

In machine learning, this is called _learning_ or _training_.
...
Predictions outside of the range are _extrapolation_.

Extrapolation has a big assumption; the data outside the training
sample will behave like the data inside the sample.
...
Can't prove causality:
  - Can provide evidence for suspected causes
  - Can identify candidate causes for later testing
